...
public class MySoldierVigiaNE extends CSoldier{
	...
	private int iAmmoThreshold;

	private int iHealthThreshold;

	protected Vector<Object> posiciones;
   ...
   protected ArrayList<Nodo> calcularVecinos(ArrayList<Nodo> lvecinos,Nodo nActual) {
	...
	protected void CreateControlPoints() {
		...
		case CLASS_SOLDIER:
			...
			int x=xini;
			int z=zini;
			while(x < 32 && z > 0 && (Math.abs(x-xini)<=3) && m_Map.CanWalk(x+1, z-1)){
				x++;
				z--;
			}
			double x2 = x*8;
